...
export function HeroLattice({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const drawReticle = (alpha: number) => {
      ctx.save();
      ctx.strokeStyle = `rgba(114, 146, 199, ${0.26 * alpha})`;
      ctx.lineWidth = 1;
      for (let i = 1; i <= 5; i++) {
        ctx.beginPath();
    ...
    const drawDots = (alpha: number) => {
      for (const dot of dots) {
        ...
        if (energy > 0.02) {
          ...
          ctx.fillStyle = `rgba(93, 192, 209, ${Math.min(0.88, energy * 0.88) * alpha})`;
          ctx.fillRect(px - size / 2, py - size / 2, size, size);
        } else {
          ctx.fillStyle = `rgba(187, 202, 228, ${base * alpha})`;
    ...
    const drawSweep = (alpha: number) => {
      ...
      grad.addColorStop(0, 'rgba(93, 192, 209, 0)');
      grad.addColorStop(0.35, `rgba(93, 192, 209, ${0.34 * alpha})`);
      grad.addColorStop(1, 'rgba(93, 192, 209, 0)');
