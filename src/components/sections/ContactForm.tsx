...
export function ContactForm() {
  ...
  if (status === 'sent' || status === 'draft') {
    return (
      <div
        role="status"
        className="relative flex min-h-[26rem] flex-col justify-center border border-border-command/45 bg-surface-intermediate p-10"
      >
        <span className="label-mono text-[0.62rem] text-accent">
          {status === 'sent' ? 'Transmission logged' : 'Email draft ready'}
        </span>
        <p className="mt-6 font-grotesk text-2xl font-bold tracking-tight text-foreground-primary">
        ...
        <button
          ...
          className="mt-8 self-start label-mono text-[0.62rem] text-foreground-secondary underline underline-offset-8 transition-colors duration-[var(--motion-micro)] hover:text-accent"
        >
          Submit another
        </button>
      </div>
      ...
      <div className="grid gap-x-8 gap-y-9 sm:grid-cols-2">
        ...
        <div data-contact-field className="group/field sm:col-span-2">
          ...
          >
            {contact.interests.map((option) => (
              <option key={option} value={option} className="bg-surface-canvas text-foreground-primary">
                {option}
              </option>
            ))}
          </select>
        ...
        <div data-contact-field className="group/field sm:col-span-2">
          ...
          <textarea
            ...
            className={cn(fieldClass, 'resize-y', errors.message && 'border-signal-bright')}
          />
          {errors.message ? (
  ...
  return (
    <div data-contact-field className="group/field">
      ...
      <input
        ...
        className={cn(fieldClass, error && 'border-signal-bright')}
      />
      {error ? (
